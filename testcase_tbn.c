 ...
 * @ Modified time: 2024-09-28 12:56:35
 * @ Description:
 */
...
int main()
{
    ...
    conv2d_layer *conv1 = create_conv2d_layer(input_channel, 64, 3, 1, 1, 1, TBN);
    model = add_layer(model, CONV, "conv1", conv1);
    conv2d_layer *conv2 = create_conv2d_layer(64, 64, 3, 1, 1, 1, TBN);
    model = add_layer(model, CONV, "conv2", conv2);
    linear_layer *linear1 = create_linear_layer(64 * (input_height / 2 / 2) * (input_width / 2 / 2), 128, TBN);
    model = add_layer(model, LINEAR, "linear1", linear1);
    linear_layer *linear2 = create_linear_layer(128, 2, TBN);
    model = add_layer(model, LINEAR, "linear2", linear2);

    FILE *file;
    int layer, inc, outc, kx, ky;
    int size;
    file = fopen("model_tbn.bin", "rb");
    if (file == NULL)
    {
    ...
    size = inc * outc * kx * ky;
    conv2->weights_b = (long *)malloc(size * sizeof(long));
    ...
    size = inc * outc;
    linear1->weights_b = (long *)malloc(size * sizeof(long));
    ...
    linear2->weights_b = (long *)malloc(size * sizeof(long));
    size = inc * outc;
    ...
    file = fopen("testcase_tbn.bin", "rb");
    if (file == NULL)
    {
