 ...
 * @ Modified time: 2024-09-28 12:56:39
 * @ Description:
 */
...
int main()
{
    ...
    conv2d_layer *conv1 = create_conv2d_layer(input_channel, 64, 3, 1, 1, 1, TNN);
    model = add_layer(model, CONV, "conv1", conv1);
    conv2d_layer *conv2 = create_conv2d_layer(64, 64, 3, 1, 1, 1, TNN);
    model = add_layer(model, CONV, "conv2", conv2);
    linear_layer *linear1 = create_linear_layer(64 * (input_height / 2 / 2) * (input_width / 2 / 2), 128, TNN);
    model = add_layer(model, LINEAR, "linear1", linear1);
    linear_layer *linear2 = create_linear_layer(128, 2, TNN);
    model = add_layer(model, LINEAR, "linear2", linear2);

    FILE *file;
    int layer, inc, outc, kx, ky;
    int size;
    file = fopen("model_tnn.bin", "rb");
    if (file == NULL)
    {
    ...
    size = inc * outc * kx * ky;
    conv1->weights_t0 = (long *)malloc(size * sizeof(long));
    conv1->weights_t1 = (long *)malloc(size * sizeof(long));
    fread(conv1->weights_t0, sizeof(long), size, file);  
    fread(conv1->weights_t1, sizeof(long), size, file);  
    printf("CONV1 layer:%d outc:%d inc:%d thres:%f\n",layer, outc, inc, conv1->input_thres);
    // LAYER CONV2 ##########################################################################################################################################################################################################################################################
    fread(&layer, sizeof(int), 1, file);
    fread(&outc, sizeof(int), 1, file);
    fread(&inc, sizeof(int), 1, file);
    ...
    size = inc * outc * kx * ky;
    fread(conv2->weights_t0, sizeof(long), size, file);
    fread(conv2->weights_t1, sizeof(long), size, file);
    printf("CONV2 layer:%d outc:%d inc:%d thres:%f\n",layer, outc, inc, conv2->input_thres);
    // LAYER LINEAR1 ##########################################################################################################################################################################################################################################################
    fread(&layer, sizeof(int), 1, file);
    fread(&outc, sizeof(int), 1, file);
    fread(&inc, sizeof(int), 1, file);
    ...
    size = inc * outc;
    fread(linear1->weights_t0, sizeof(long), size, file);
    fread(linear1->weights_t1, sizeof(long), size, file);
    printf("LINEAR1 layer:%d outc:%d inc:%d thres:%f\n",layer, outc, inc, linear1->input_thres);
    // LAYER LINEAR2 ##########################################################################################################################################################################################################################################################
    fread(&layer, sizeof(int), 1, file);
    fread(&outc, sizeof(int), 1, file);
    fread(&inc, sizeof(int), 1, file);
    ...
    size = inc * outc;
    fread(linear2->weights_t0, sizeof(long), size, file);
    fread(linear2->weights_t1, sizeof(long), size, file);
    printf("LINEAR2 layer:%d outc:%d inc:%d thres:%f\n",layer, outc, inc, linear2->input_thres);
    int num_tesecase;
    int num_correct = 0;
    
    file = fopen("testcase_tnn.bin", "rb");
    if (file == NULL)
    {
    ...
    printf("\n BEGIN %d TESCASES FOR TBN MODEL ....\n", num_tesecase);
