 ...
 * @ Modified time: 2024-09-28 13:33:11
 * @ Description:
 */
...
int main()
{
    ...
    // int t = 1;
    {
        ...
        conv2d_layer *conv0 = create_conv2d_layer(1, 64, 3, 1, 1, 1, typ[t]);
        model = add_layer(model, CONV, "conv0", conv0);
        conv2d_layer *conv1 = create_conv2d_layer(64, 64, 3, 1, 1, 1, typ[t]);
        ...
        for (int ct = 0; ct < NO_TESTS; ct++)
        {
            ...
            float *x0 = conv2d_forward(conv0, input, input_height, input_width);
            float *x1 = conv2d_forward(conv1, x0, input_height, input_width);
        ...
        printf("Model execution time  %d: %.6f seconds, %.4f\n\n", t, time_taken/NO_TESTS, time_FP/time_taken);
